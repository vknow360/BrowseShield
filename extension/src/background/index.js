...
import { initVisionPipeline, detectFaces, perceiveScreen } from './vision-pipeline.js';
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'ping') {
    ...
    const { sanitizedPayload, taskInstruction, actionHistory, tokenTypes } = message.payload;
      ...
      try {
        ...
        if (tabId !== null && windowId !== null) {
          try {
            ...
            const dataUrl = await browser.tabs.captureVisibleTab(windowId, { format: 'jpeg', quality: 80 });
            const blob = await (await fetch(dataUrl)).blob();
            const fullBitmap = await createImageBitmap(blob);

            // Downscale once (longest edge <= 768) to massively shrink inference, redaction, and upload.
            const MAX_EDGE = 768;
            ...
            const perception = await perceiveScreen(imageBitmap, sanitizedPayload.nodes);
            ...
            sanitizedPayload.nodes.forEach(node => {
              if (node.box && Array.isArray(node.box) && node.box.length === 4) {
                const boxObj = {
                  x: node.box[0] * f, y: node.box[1] * f,
                  w: node.box[2] * f, h: node.box[3] * f
                };
            ...
            imageBitmap.close();
            console.log(`[ShieldBrowse SW] Visual loop complete. ScreenType: ${screenType} | `
              + `capture ${timing.captureMs}ms, perceive ${timing.perceiveMs}ms, redact ${timing.redactMs}ms`);
          } catch (visionErr) {
            console.error('[ShieldBrowse SW] Vision loop failed, continuing text-only:', visionErr);
          }
        }
        ...
        const tokenMap = (await browser.storage.local.get('tokenMap')).tokenMap || {};
        ...
        try {
          ...
          console.warn('[ShieldBrowse SW] Could not import privacyGate; relying on content-script gate.', importErr);
        }
        if (privacyGate) {
          const gate = privacyGate(reqBody, tokenMap, 'block');
          if (!gate.allowed) {
            throw new Error(`Privacy Gate blocked network egress: raw PII detected (${gate.violations.map(v => v.token).join(', ')})`);
          }
        }
        ...
        timing.totalMs = (timing.captureMs || 0) + (timing.perceiveMs || 0) + (timing.redactMs || 0) + timing.networkMs;

        console.log(`[ShieldBrowse SW] Server action received in ${timing.networkMs}ms `
          + `(end-to-end ~${timing.totalMs}ms):`, actionJSON);
