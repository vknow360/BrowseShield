...
export async function initVisionPipeline() {
  ...
  try {
    const vision = await FilesetResolver.forVisionTasks(browser.runtime.getURL('wasm'));
    ...
    try {
      ...
      console.error('[Vision] YOLOv8 model failed to load — UI element perception disabled. '
        + 'Ensure public/models/yolov8n.onnx is present.', e);
    }
...
export async function perceiveScreen(imageBitmap, nodes = []) {
  ...
  try {
    if (yoloSession) {
      ...
      uiBoxes = postprocessYOLO(outputTensor, scale, offsetX, offsetY, imageBitmap.width, imageBitmap.height);
      tensor.dispose();
    }
