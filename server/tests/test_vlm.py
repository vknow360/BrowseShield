...
def test_redacted_image_is_sent_to_ollama(monkeypatch):
    ...
    action = asyncio.run(vlm_service.get_next_action_from_vlm(req))

    assert isinstance(action, AgentAction)
    user_msg = captured["payload"]["messages"][-1]
    assert user_msg["role"] == "user"
    assert user_msg.get("images") == ["QUJD"]  # data-URI prefix stripped
...
def test_no_image_means_no_images_field(monkeypatch):
    ...
    asyncio.run(vlm_service.get_next_action_from_vlm(req))
...
def test_mock_fallback_when_ollama_unreachable(monkeypatch):
    ...
    action = asyncio.run(vlm_service.get_next_action_from_vlm(AgentRequest(taskInstruction="Fill")))
    assert "[MOCK/OLLAMA-DOWN]" in action.reasoning
...
def test_action_endpoint_returns_valid_action(monkeypatch):
    async def fake_vlm(req):
        return AgentAction(action="done", reasoning="Task completed")

    monkeypatch.setattr("app.api.agent_router.get_next_action_from_vlm", fake_vlm)
...
def test_action_endpoint_surfaces_errors(monkeypatch):
    ...
    monkeypatch.setattr("app.api.agent_router.get_next_action_from_vlm", boom)
